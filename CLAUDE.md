...
**Audio narration (engine v2)** — every diagram and graph player exposes two always-on buttons: 🔊 (current step) and ▶ (play all steps in sequence, advancing the visuals). Narration is per-step: step 1 prefixes the diagram's `description` / `audioDesc` field, subsequent steps speak only the step caption. After the caption, `_naStructure` appends a machine-built sentence describing visible elements in reading order, arrow connections, and highlights — works on every diagram/graph with zero teacher effort (WCAG 2.1 §1.1.1). The unified engine (`_naSpeak`, `_naStepText`, `_naPlayAll`) routes through `preprocessSpeechText` (crypto-aware pronunciation dictionary, including domain URLs) and `getBestVoice` — the same pipeline as flashcards. Speech cancels on stage change (`renderLessonStage`) and on manual step navigation (`idiagNav`, `igraphNav`). xAPI emits one `interacted · Audio narration: <title> (step N/M)` event per listen; `_naBumpStat` writes a persistent count to `cc_narration_stats` for the teacher Analytics panel.
...
- **Speed preference:** `cc_tts_rate` (localStorage) — values 0.75 / 0.90 / 1.10, clamped 0.6–1.4, set via the "🔊 Audio & Accessibility" card on the Profile screen (`setTTSRate`, `_syncTTSRateUI`). Applied to diagram, graph, and flashcard narration via `_naGetRate()`. **Reserved key — do not repurpose.**
- **Listen statistics:** `cc_narration_stats` (localStorage) — `{ [title]: { count, last } }`, written by `_naBumpStat` on every 🔊 and ▶ listen. Rendered in the teacher Analytics tab as a ranked bar list. Uncapped (unlike the 200-event xAPI ring buffer). **Reserved key — do not repurpose.**
